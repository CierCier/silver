...
fn corpus() -> Vec<(String, String)> {
    ...
    let root = manifest_dir
        .ancestors()
        .find(|candidate| candidate.join("std").is_dir() && candidate.join("Cargo.toml").is_file())
        .expect("repository root containing std/ and Cargo.toml");
    ...
    group.bench_function(name.to_string(), |b| {
        b.iter(|| {
            for (_, src) in sources {
                ...
                let buf = elise_lex::scan(&mut spec, std::hint::black_box(src)).expect("lex");
...
fn bench_elise_pipeline_group(c: &mut Criterion, name: &str, sources: &[(&str, &str)]) {
    let (bytes, _lines) = bytes_and_lines(sources);
    if bytes == 0 {
        return;
    }
    let mut group = c.benchmark_group(format!("elise_pipeline/{name}"));
    group.throughput(Throughput::Bytes(bytes));
    group.bench_function(name.to_string(), |b| {
        b.iter(|| {
            for (_, src) in sources {
                let graph = agc::grammar::parse_ag(std::hint::black_box(src));
                std::hint::black_box(graph.text().len());
            }
        })
    });
    group.finish();
...
fn corpus_benches(c: &mut Criterion) {
    ...
    for (prefix, label) in [("std", "std"), ("tests", "tests"), ("examples", "examples")] {
        ...
        bench_elise_pipeline_group(c, label, &sources);
    }
    bench_elise_lex_group(c, "all", &all);
    bench_elise_pipeline_group(c, "all", &all);
