...
impl ModuleLoader {
    ...
    pub fn compute_cache_key_with_deps(
        ...
        builder.add_target(&self.effective_target());
        builder.add_opt_level(self.opt_level.as_deref());
        builder.add_flags(&self.cfg_flags);
        builder.add_str("emit_mode");
        builder.add_str(&self.emit_mode);
        builder.add_bool("tree_shake", self.tree_shake);
        builder.add_bool("test_harness", self.test_harness);
        if self.debug_info {
...
mod tests {
    ...
    fn cache_source(label: &str) -> PathBuf {
        let root = unique_temp_dir(label);
        std::fs::create_dir_all(&root).unwrap();
        let source = root.join("main.ag");
        std::fs::write(&source, "i32 main() { return 0; }\n").unwrap();
        source
    }

    #[test]
    fn cache_key_changes_with_emit_mode() {
        let source = cache_source("cache-key-emit-mode");
        let mut loader = ModuleLoader::new();
        let baseline = loader.compute_cache_key(&source, "main").unwrap();
        loader.emit_mode = "Obj".to_string();
        let changed = loader.compute_cache_key(&source, "main").unwrap();
        assert_ne!(baseline, changed);
        let _ = std::fs::remove_dir_all(source.parent().unwrap());
    }

    #[test]
    fn cache_key_changes_with_tree_shake_and_test_harness() {
        let source = cache_source("cache-key-build-flags");
        let mut loader = ModuleLoader::new();
        let baseline = loader.compute_cache_key(&source, "main").unwrap();
        loader.tree_shake = true;
        let shaken = loader.compute_cache_key(&source, "main").unwrap();
        loader.tree_shake = false;
        loader.test_harness = true;
        let harnessed = loader.compute_cache_key(&source, "main").unwrap();
        assert_ne!(baseline, shaken);
        assert_ne!(baseline, harnessed);
        let _ = std::fs::remove_dir_all(source.parent().unwrap());
    }

    #[test]
    fn cache_key_changes_with_target() {
        let source = cache_source("cache-key-target");
        let mut loader = ModuleLoader::new();
        let baseline = loader.compute_cache_key(&source, "main").unwrap();
        loader.target = Some("test-target".to_string());
        let changed = loader.compute_cache_key(&source, "main").unwrap();
        assert_ne!(baseline, changed);
        let _ = std::fs::remove_dir_all(source.parent().unwrap());
    }
