...
pub const RUNTIME_ENTRY_API_MODULE: &str = "std.sys.entry_api";

/// Resolve the import used by a compilation unit. Non-root units receive the
/// runtime ABI declarations rather than a second copy of the entry storage.
pub fn effective_import_path(
    path: &[ast::Identifier],
    include_entry_import: bool,
) -> (String, Vec<ast::Identifier>) {
    let requested = import_path_to_string(path);
    if !include_entry_import && requested == RUNTIME_ENTRY_MODULE {
        return (
            RUNTIME_ENTRY_API_MODULE.to_string(),
            vec![
                ast::Identifier {
                    name: "std".to_string(),
                ...
                ast::Identifier {
                    name: "entry_api".to_string(),
                    span: lexer::Span::default(),
                },
            ],
        );
    }
    (requested, path.to_vec())
}
...
impl<'a> FileImportResolverHook<'a> {
    ...
    fn lower_program_recursive(
        ...
        for item in std::mem::take(&mut program.items) {
            ...
            // A cached dependency object must not inline the executable entry
            // implementation: it owns private argc/argv/envp storage that its
            // own _start never initializes. Library/module builds get the ABI
            // declarations instead; the root build owns the implementation.
            let (module_path, effective_import_path) =
                effective_import_path(&import_item.path, self.include_entry_import);
            ...
            let resolved = self
                .loader
                .find_source_import(&effective_import_path, base_dir)
