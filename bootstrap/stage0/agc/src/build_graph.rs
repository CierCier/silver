...
pub struct DependencyGraph {
    ...
    /// Source paths for all discovered modules, including source-inlined and binary imports.
    pub source_paths: HashMap<String, PathBuf>,
    /// Direct import edges for every discovered module, including inlined and binary ones.
...
impl DependencyGraph {
    pub fn new() -> Self {
        Self {
            ...
            source_dependencies: HashMap::default(),
        }
    }
    ...
    pub fn build(loader: &ModuleLoader, root_inputs: &[PathBuf]) -> Result<Self, String> {
        ...
        for input in root_inputs {
            graph.discover_module(loader, input, None, &mut visited, true)?;
        }
    ...
    fn discover_module(
        ...
        if !visited.insert(canonical.clone()) {
            if let Some(module_path) = &module_path {
                self.source_paths
                    ...
                    .or_insert_with(|| source_path.to_path_buf());
            }
        ...
        for item in &ast.items {
            ...
            let (dep_path_str, dep_import_path) = crate::parser::import_hook::effective_import_path(
                &import_item.path,
                allow_entry_import,
            );
            if let Some(resolved) =
                loader.find_source_import(&dep_import_path, source_path.parent())
            {
                match resolved.kind {
                    ...
                    ResolvedSourceImportKind::Module => {
                        self.source_paths
                            .entry(dep_path_str.clone())
                            .or_insert(resolved.source_path);
                        direct_deps.push(dep_path_str);
                    }
                }
            }
        }

        let recorded_deps = self
            .source_dependencies
            .entry(current_module_path.clone())
            .or_default();
        recorded_deps.extend(direct_deps.iter().cloned());
        recorded_deps.sort();
        recorded_deps.dedup();
    ...
    fn source_dependency_closure(&self, module_path: &str) -> Vec<String> {
        let mut pending = vec![module_path.to_string()];
        let mut visited = HashSet::default();
        let mut dependencies = Vec::new();

        while let Some(current) = pending.pop() {
            let Some(direct) = self.source_dependencies.get(&current) else {
                continue;
            };
            for dependency in direct {
                if dependency == module_path || !visited.insert(dependency.clone()) {
                    continue;
                }
    ...
    fn cache_dependency_hashes(
        &self,
        loader: &ModuleLoader,
        node: &ModuleNode,
        module_keys: &HashMap<String, CacheKey>,
    ) -> Result<Vec<(String, String)>, String> {
        let mut dependency_names = node.dependencies.clone();
        dependency_names.extend(self.source_dependency_closure(&node.module_path));
            ...
            .map(|dependency| {
                let key = module_keys
                    ...
                    .or_else(|| {
                        self.source_paths
                            .get(&dependency)
                            .and_then(|path| loader.compute_cache_key(path, &dependency))
                    })
                    .ok_or_else(|| {
                        msg::cache_dependency_key_failed(&dependency, &node.module_path)
                    })?;
                Ok((dependency, key.hash_hex))
            })
            .collect()
    }
}
...
mod tests {
    ...
    #[test]
    fn cache_key_tracks_binary_imports() {
        let root = std::env::temp_dir().join(format!(
            "agc-build-graph-binary-cache-{}-{}",
            std::process::id(),
        ...
        std::fs::write(std_root.join("binary.agm"), b"artifact-v1").unwrap();
        std::fs::write(std_root.join("ops.ag"), "import std.binary;\n").unwrap();
        std::fs::write(root.join("main.ag"), "import std.ops;\n").unwrap();

        let mut loader = ModuleLoader::new();
        loader.add_search_dir(&root);
        let graph = DependencyGraph::build(&loader, &[root.join("main.ag")]).unwrap();
        let node = graph.nodes.get("std.ops").expect("cacheable ops module");
        let before_deps = graph
            .cache_dependency_hashes(&loader, node, &HashMap::default())
            .unwrap();
        let before = loader
            .compute_cache_key_with_deps(&node.source_path, &node.module_path, &before_deps)
            .unwrap();

        assert!(before_deps.iter().any(|(name, _)| name == "std.binary"));
        std::fs::write(std_root.join("binary.agm"), b"artifact-v2").unwrap();
        let after_deps = graph
            .cache_dependency_hashes(&loader, node, &HashMap::default())
            .unwrap();
        let after = loader
            .compute_cache_key_with_deps(&node.source_path, &node.module_path, &after_deps)
            .unwrap();
    ...
    #[test]
    fn cache_key_tracks_transitive_inlined_source_dependencies() {
        let root = std::env::temp_dir().join(format!(
            "agc-build-graph-cache-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
        ...
        let std_root = root.join("std");
        std::fs::create_dir_all(&std_root).unwrap();
        std::fs::write(std_root.join("leaf.ag"), "i32 leaf() { return 1; }\n").unwrap();
        std::fs::write(std_root.join("dep.ag"), "import std.leaf;\n").unwrap();
        std::fs::write(std_root.join("ops.ag"), "import std.dep;\n").unwrap();
        std::fs::write(root.join("main.ag"), "import std.ops;\n").unwrap();
        ...
        assert!(before_deps.iter().any(|(name, _)| name == "std.dep"));
        assert!(before_deps.iter().any(|(name, _)| name == "std.leaf"));

        std::fs::write(std_root.join("leaf.ag"), "i32 leaf() { return 2; }\n").unwrap();
        let after_deps = graph
            .cache_dependency_hashes(&loader, node, &HashMap::default())
            .unwrap();
        let after = loader
    ...
    #[test]
    fn cache_key_uses_entry_api_for_dependency_imports() {
        let root = std::env::temp_dir().join(format!(
            "agc-build-graph-entry-api-cache-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
        ...
        assert!(deps.iter().any(|(name, _)| name == "std.sys.entry_api"));
        assert!(!deps.iter().any(|(name, _)| name == "std.sys.entry"));
        let _ = std::fs::remove_dir_all(root);
    }
}
