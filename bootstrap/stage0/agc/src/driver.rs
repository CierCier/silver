...
fn derive_plan(cli: Cli) -> Result<CompilePlan, String> {
            ...
            .unwrap_or(false)
    {
        cli.bin
            ...
            .and_then(|value| {
                ...
                } else if package::is_manifest_candidate(&path) {
                    path
                } else {
                    return None;
                };
    ...
    ) && !cli.clean
    {
        match inputs.as_slice() {
            ...
            [input] if package::is_manifest_candidate(input) => Some(input.clone()),
            _ => None,
        }
    } else {
        None
    ...
    if (cli.bin.is_some() || cli.lib.is_some()) && package_candidate.is_none() {
        return Err("--bin and --lib require a package directory or manifest input".to_string());
    }
...
fn build_module_loader(plan: &CompilePlan) -> ModuleLoader {
    ...
    loader.emit_mode = format!("{:?}", plan.emit);
    loader.tree_shake = plan.tree_shake;
    loader.test_harness = plan.test_harness;
    ...
    for package_root in &plan.package_roots {
        ...
        if package_src.is_dir() {
            loader.add_search_dir(package_src);
        }
    ...
    match derive_plan(cli) {
        Ok(mut plan) => {
            ...
            for input in &plan.inputs {
                ...
                // Compile-time cfg gate: expand top-level `if (@cfg(...))`
                // blocks (defensive: import lowering already expanded them so
                // dead imports never resolved), drop #[cfg(...)]-rejected
                // items, then fold @cfg(...) and prune dead branches, before
                // any symbol registration, semantic analysis, or type checking
                // sees them.
                ...
                let mut cfg_errors = cfg::expand_cfg_blocks(&mut ast, &cfg_set);
                cfg_errors.extend(cfg::gate_items(&mut ast, &cfg_set));
                ...
                if matches!(
                    ...
                    if matches!(plan.emit, EmitKind::Obj) {
                        ...
                        if !plan.no_cache && ROOT_OBJECT_CACHE_ENABLED {
                            if let Some(store) = &loader.cache_store {
