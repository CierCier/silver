...
impl<'ctx> LlvmIrGenerator<'ctx> {
    ...
    pub(crate) fn declare_function_item(
        ...
        if link_name.is_some() {
            self.root_symbols.insert(llvm_name.to_string());
            self.root_symbols.insert(func.name.name.clone());
            // A link_name is an external ABI contract. Keep the definition in
            // the object even when the current translation unit has no direct
            // call; another linked module may reference it.
            self.keep_items.push(llvm_name.to_string());
