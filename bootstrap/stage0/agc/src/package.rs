...
/// Return whether a command-line path names a manifest rather than a source
...
fn resolve_manifest_path(base_dir: &Path, path: &Path) -> Result<PathBuf, PackageError> {
    ...
    let candidate = if joined.is_file() {
        // A manifest may have any filename (for example `bin/agc`), not
        // only the conventional `silver.toml` name.
        joined
    } else if path.file_name().and_then(|name| name.to_str()) == Some(MANIFEST_FILE) {
        joined
    } else {
        // Preserve directory-style references and the old missing-path
        // diagnostic for paths that are intended to contain silver.toml.
        joined.join(MANIFEST_FILE)
    };
...
"#,
    ...
    #[test]
    fn resolves_arbitrary_manifest_files_for_targets() {
        let root = unique_temp_dir("arbitrary-manifest");
        let nested_dir = root.join("bin");
        fs::create_dir_all(nested_dir.join("src")).unwrap();
        let nested_manifest = nested_dir.join("agc");
        fs::write(
            &nested_manifest,
            "name = \"nested\"\nversion = \"0.1.0\"\n\n[bin.agc]\nentry = \"src/main.ag\"\n\n[lib.agc]\nentry = \"src/lib.ag\"\n",
        )
        .unwrap();
        fs::write(nested_dir.join("src/main.ag"), "i32 main() { return 0; }\n")
            .unwrap();
        fs::write(nested_dir.join("src/lib.ag"), "i32 answer() { return 42; }\n")
            .unwrap();
        ...
        let graph = resolver.resolve(&root_manifest).unwrap();
        let binary = graph
            .select_target(&TargetSelection::bin(Some("agc".to_string())))
            .unwrap();
        let library = graph
            .select_target(&TargetSelection::lib(Some("agc".to_string())))
            .unwrap();
        let expected_binary = fs::canonicalize(nested_dir.join("src/main.ag")).unwrap();
        let expected_library = fs::canonicalize(nested_dir.join("src/lib.ag")).unwrap();
        assert_eq!(binary.entry, expected_binary);
        assert_eq!(library.entry, expected_library);
