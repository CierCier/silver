...
impl<'a, 'e> BodyParser<'a, 'e> {
    ...
    fn looks_like_local_decl(&self) -> bool {
        ...
        while i < limit {
            ...
            match tok {
                // The declarator name: an identifier at angle-depth 0
                // after at least one type token.
                Tok::Ident if angle == 0 && seen_type_start => {
                    let j = self.skip_trivia(i + 1);
                ...
                Tok::Mut | Tok::Const | Tok::Static | Tok::Volatile | Tok::Star => {}
                Tok::Less => angle += 1,
    ...
    fn primary(&mut self) {
        match self.peek_tok() {
            ...
            Some(Tok::LParen) => {
                self.enter(BodyKind::ExprParen as u16);
                self.advance(); // (
                self.expr();
    ...
    fn cast_paren_ahead(&self) -> bool {
        ...
        while i < window_end {
            ...
            match tok {
                ...
                Tok::LParen => depth += 1,
                Tok::Mut | Tok::Const | Tok::Volatile | Tok::Star => {}
                _ => {
                    typeish += 1;
                }
            }
