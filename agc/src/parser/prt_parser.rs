...
impl TransitionTable {
    ...
    fn predict_item(&self, lookahead: &[TokenClass]) -> Option<ItemProduction> {
...
impl PRT_Parser {
    ...
    pub fn grammar_mut(&mut self) -> &mut GrammarSpec {
        &mut self.grammar
    }
    ...
    fn install_bootstrap_grammar(&mut self) {
        ...
        self.grammar.add_rule(
            "ExprStmt",
            seq(vec![rule("Expression"), tok(Token::Semicolon)]),
        );
    ...
    fn token_class_with_context(&self, tokens: &[LexToken], index: usize) -> Option<TokenClass> {
        ...
        match &token.kind {
            ...
            {
                Some(TokenClass::TypeStart)
            }
            _ if Self::is_type_start_token(&token.kind)
                && !matches!(token.kind, Token::Identifier(_)) =>
            {
                Some(TokenClass::TypeStart)
            }
            _ => None,
    ...
    fn classify_identifier_as_type_start(&self, tokens: &[LexToken], index: usize) -> bool {
    ...
    fn looks_like_function_item_start(&self, tokens: &[LexToken], start: usize) -> bool {
        ...
        if matches!(
            ...
            let Some(close) =
                self.find_matching_token(tokens, cursor, tokens.len(), Token::Less, Token::Greater)
            else {
                return false;
            };
            cursor = close + 1;
    ...
    fn lookahead_classes(&self, tokens: &[LexToken], start: usize) -> Vec<TokenClass> {
        ...
        for (offset, token) in tokens.iter().enumerate().skip(start) {
            ...
            if let Some(class) = self.token_class_with_context(tokens, offset) {
                classes.push(class);
                if classes.len() >= self.transition_table.max_lookahead {
                    break;
                }
            } else {
                break;
    ...
    fn parse_block_reduction(
        ...
        while cursor + 1 < end {
            ...
            if matches!(tokens[cursor].kind, Token::If) {
                ...
                let then_start = cond_close + 1;
                let Some(then_end_inclusive) = self.find_matching_token(
                    tokens,
                    then_start,
                    end,
                    Token::LeftBrace,
        ...
        while cursor < end {
            match &tokens[cursor].kind {
                ...
                Token::StringLiteral(value) => {
                    args.push(ast::AttributeArg::Literal(ast::Literal::String(
                        value.clone(),
                    )));
                    cursor += 1;
                }
    ...
    fn parse_attributes_prefix(
        ...
        while cursor + 2 < end && matches!(tokens[cursor].kind, Token::Hash) {
            ...
            let Some(bracket_end) = self.find_matching_token(
                tokens,
                bracket_start,
                end,
                Token::LeftBracket,
    ...
    fn parse_trait_reduction(
        ...
        if cursor < end && matches!(tokens[cursor].kind, Token::Colon) {
            ...
            loop {
                ...
                let mut bound_end = cursor;
                while bound_end < end
                    && !matches!(tokens[bound_end].kind, Token::Comma | Token::LeftBrace)
                {
                    bound_end += 1;
                }
                if bound_start == bound_end {
            ...
            loop {
                ...
                while bound_end < end
                    && !matches!(
                        ...
                        Token::Plus | Token::Assign | Token::Semicolon
                    )
                {
                    bound_end += 1;
                }
    ...
    fn parse_extern_member_reduction(
        ...
        while cursor < end {
            ...
            let (parameter_name, parameter_span, parameter_end) =
                if let Some(param_name_token) = tokens.get(cursor) {
                    if let Token::Identifier(parameter_name) = &param_name_token.kind {
                        cursor += 1;
                        (
    ...
    pub fn parse_program(&mut self, tokens: &[LexToken]) -> Result<ast::Program, ParseError> {
        ...
        while !Self::at_eof(tokens, position) {
            let (attributes, item_start) =
                self.parse_attributes_prefix(tokens, position, tokens.len())?;
...
mod tests {
    ...
    #[test]
    fn parses_extern_function_without_explicit_abi() {
        let source = "extern i32 puts(str);";
        let tokens = lex(source).expect("lex failed");
        let mut parser = PRT_Parser::new(None);
        let program = parser.parse_program(&tokens).expect("parse failed");
        assert_eq!(program.items.len(), 1);
        let ast::ItemKind::ExternFunction(func) = &program.items[0].kind else {
    ...
    #[test]
    fn parses_extern_variable_declaration() {
        let source = "extern i32 errno;";
        let tokens = lex(source).expect("lex failed");
        let mut parser = PRT_Parser::new(None);
        let program = parser.parse_program(&tokens).expect("parse failed");
        assert_eq!(program.items.len(), 1);
    ...
    fn tracks_type_names_for_statement_disambiguation() {
        ...
        assert!(matches!(
            main_fn.body.statements[0].kind,
            ast::StatementKind::Let(_)
        ));
        assert!(matches!(
