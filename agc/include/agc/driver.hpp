...
public:
    ...
    // Run the compilation pipeline for a single file (internal use mostly, but exposed)
...
private:
    ...
    std::optional<std::string> resolve_import(const std::vector<std::string>& parts, const std::vector<std::string>& include_paths);
